//Create an object that represents a bank with one account.
...
$(document).ready(function() {
  ...
  let theBank = {
    ...
    //Created function that handled the GUI actions for deposit and withdraw.

    action: function() {
      if ($deposit.hasClass("clicked")) { //if the deposit button is pressed create deposit.
        if ($credit < 100) { //If credit account is depleated, deposit into credit account.
          $credit += parseInt(amount);
          $("#creditBalance").html("$" + $credit);
        } else { //else deposit into the savings account.
          $savings += parseInt(amount);
          $("#savingsBalance").html("$" + $savings);
          console.log($savings);
        }
      }
      if ($withdraw.hasClass("clicked")) { // IF the withdraw button is pressed, create a withdraw.
        if (amount > $savings) {
          if (amount > $credit) {
          ...
          $savings -= parseInt(amount); // ParseFloat
          $("#savingsBalance").html("$" + $savings);
          console.log($savings);
        }
      }
    ...
    //Created function to animate the page with GIFs that are called in at certain account balance intervals.
